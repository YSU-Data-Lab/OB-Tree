...
public class exp_merge_progressive_select {
		...
		for(double per:pers){
			...
			for(int index=0;index<=appendix_num_split;index++){
				...
				for(int target_oid:select_list){
					long target_value= TBAT.selectTBAT_Uncleaned_Split(tbat_file_name,
							appendix_file_names, num_lines, tbat_line_length, target_oid);
				}
	...
	public static void prepareFiles() throws IOException {
		//-----prepare tbat and bat files-----
				...
				for(double per:pers){
					...
					DataCreator.creaetTBATAppendix(update_file_name, appendix_file_prefix,appendix_block_size);
				}
